"""Mixin and base class for DupleBalanceClassifier.
"""
...
class ImbalancedEnsembleClassifierMixin(ClassifierMixin):
    """Mixin class for DupleBalanceClassifier.

    This class provides several utilities for formatting training logs.
        ...
        if return_value_dict == True:
            ...
            for data_name, (X_eval, y_eval) in eval_datasets_.items():
                ...
                for metric_name, (metric_func, args) in eval_metrics.items():
                    try:
                        score = metric_func(y_eval, y_predict_proba, **args)
                    except:
